...
export default function fetchSortData(rankRef:MutableRefObject<Array<[string|number]>>, num:number, passportDataRef:MutableRefObject<Array<[string|number]>>) {
  ...
  //South Ossetia removing singular E-visa access
  rankData[175][4] = 0

  const passportDataArray:Array<[string|number]> = []
  const passportDataIndexes:number[] = [7,146,52,68,85,54,34,145,25,201,8,22]
